...
from losses import DistillDiffPruningFLOPsLoss
...
def get_args_parser():
    ...
    parser.add_argument('--accu', action='store_true', default=False, help='accumulate cls attention')
    parser.add_argument('--ema_accu', action='store_true', default=False, help='ema_accumulate cls attention')
...
def main(args):
    ...
    if args.distill:
        if 'lvvit' in args.arch:
            criterion = DistillDiffPruningFLOPsLoss(
                model_t, criterion, clf_weight=1.0, keep_ratio=KEEP_RATE, ratio_weight=args.ratiow, distill_weight=args.distillw, token_dis=True
            )
        else:
            criterion = DistillDiffPruningFLOPsLoss(
