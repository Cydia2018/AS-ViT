...
from utils import batch_index_select, LTP
...
class Attention(nn.Module):
    ...
    def forward(self, x, policy, padding_mask=None):
        ...
        attn_out = attn @ v
        x = attn_out.transpose(1, 2).reshape(B, N, self.head_dim* self.num_heads)
        x = self.proj(x)
        x = self.proj_drop(x)
...
class Block(nn.Module):
    ...
    def forward(self, x, policy=None, padding_mask=None):
        tmp, cls_attn = self.attn(self.norm1(x), policy, padding_mask)
        ...
        if self.training:
            flops = self.flops(policy.squeeze(dim=-1).sum(dim=-1))
            return x, cls_attn, flops
        return x, cls_attn
    ...
    def flops(self, num_tokens):
        ...
        # mha_block_flops
        kqv_flops = 3 * dim * dim * num_tokens
        attn_flops = dim * num_tokens * num_tokens
        attnsoftmax_flops = SOFTMAX_FLOPS * num_tokens * num_tokens * heads
        # attndropout_flops = DROPOUT_FLOPS * num_tokens * num_tokens * heads
        attndropout_flops = 0
        attnscale_flops = num_tokens * num_tokens * heads
        attnv_flops = dim * num_tokens * num_tokens
        attnout_flops = dim * dim * num_tokens + dim * num_tokens
        # attnoutdrop_flops = DROPOUT_FLOPS * dim * num_tokens
        attnoutdrop_flops = 0
        attnres_flops = dim * num_tokens
        attnln_flops = LAYER_NORM_FLOPS * dim * num_tokens
        mha_block_flops = kqv_flops + attn_flops + attnv_flops + attnsoftmax_flops + attndropout_flops + attnscale_flops + attnoutdrop_flops + attnres_flops + attnln_flops + attnout_flops
        # ffn_flops
        fc1_flops = dim * mlp_dim * num_tokens
        fc1act_flops = ACTIVATION_FLOPS * mlp_dim * num_tokens
        fc1bias_flops = mlp_dim * num_tokens
        fc2_flops = dim * num_tokens * mlp_dim
        fc2bias_flops = dim * num_tokens
        # fc2drop_flops = DROPOUT_FLOPS * dim * num_tokens
        fc2drop_flops = 0
        fc2res_flops = dim * num_tokens
        fc2ln_flops = LAYER_NORM_FLOPS * dim * num_tokens
        ffn_flops = fc1_flops + fc2_flops + fc1act_flops + fc1bias_flops + fc2bias_flops + fc2drop_flops + fc2res_flops + fc2ln_flops
        return (mha_block_flops + ffn_flops) / 1e+9
...
class PatchEmbed4_2(nn.Module):
    ...
    def flops(self):
        ...
        return sum(block_flops.values()) / 1e+9
...
class LVViTDiffPruning(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 accu=False, ema_accu=False, viz_mode=False):
        super().__init__()
        self.img_size = img_size
        self.beta = 0.99
        self.target_index = [i for i in range(2, depth)]    # 2
        ...
        self.accu = accu
        self.ema_accu = ema_accu
        if accu:
            threshold_init = [0.004, 0.012, 0.02]
        else:
            threshold_init = [0.001, 0.002, 0.003]
        
        print('## threshold_init: ', end='')
        print(threshold_init)
        predictor_list = [AdaptiveSparsityModule(threshold_init[i]) for i in range(len(pruning_loc))]
    ...
    def no_weight_decay(self):
        d = {'pos_embed', 'cls_token'}
        for i in range(len(self.pruning_loc)):
            d.add('score_predictor.{}.keep_threshold'.format(i))
        return d
    ...
    def forward(self, x):
        ...
        p_count = 0
        total_flops = self.patch_embed.flops()
        global_cls_attn = 0
        out_pred_prob = []
        init_n = 14 * 14
        ...
        for i, blk in enumerate(self.blocks):
            if i in self.pruning_loc:
                if self.training:
                    score, hard_keep_decision = self.score_predictor[p_count](global_cls_attn, tau=1e-4)
                    hard_keep_decision = hard_keep_decision[:, :, None]
                    out_pred_prob.append(hard_keep_decision.reshape(B, init_n))
                    cls_policy = torch.ones(B, 1, 1, dtype=hard_keep_decision.dtype, device=hard_keep_decision.device)
                    policy = torch.cat([cls_policy, hard_keep_decision], dim=1)
                    x, attn, flops = blk(x, policy=policy)
                    total_flops += flops
                    prev_decision = hard_keep_decision
                else:
                    if B==1:
                        score, mask = self.score_predictor[p_count](global_cls_attn, tau=1)
                        keep_policy = torch.nonzero(mask.squeeze()).squeeze().unsqueeze(dim=0)
                    else:
                        score, remain_num = self.score_predictor[p_count](global_cls_attn, tau=1)
                        num_keep_node = remain_num
                    ...
                    x, attn = blk(x)
                    global_cls_attn = batch_index_select(global_cls_attn, keep_policy)
                p_count += 1
            else:
                if self.training:
                    x, attn, flops = blk(x, policy)
                    total_flops += flops
                else:
                    x, attn = blk(x)
            ...
            if i in self.target_index:
                if self.accu:
                    global_cls_attn += attn
                elif self.ema_accu:
                    if i==self.target_index[0]:
                        global_cls_attn = attn
                    else:
                        global_cls_attn = self.beta*global_cls_attn + (1-self.beta)*attn
                else:
                    global_cls_attn = attn
        ...
        if self.training:
            if self.distill:
                return x_cls, x_aux, prev_decision.detach(), out_pred_prob, total_flops
            else:
